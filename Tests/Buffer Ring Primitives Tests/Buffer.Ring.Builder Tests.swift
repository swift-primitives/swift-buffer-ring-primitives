...
extension RingBuilderTests {
    fileprivate static func collected(
        _ buffer: consuming Buffer<Int>.Ring
    ) -> [Int] {
    ...
    fileprivate static func collected(
        _ buffer: consuming Buffer<Move>.Ring
    ) -> [Int] {
        var rest = consume buffer
        var result: [Int] = []
        while !rest.isEmpty {
...
extension RingBuilderTests.Unit {
    ...
    func `Single element expression`() {
        let buffer: Buffer<Int>.Ring = Buffer<Int>.Ring { 42 }
    ...
    func `Optional element - some`() {
        let value: Int? = 42
        let buffer: Buffer<Int>.Ring = Buffer<Int>.Ring { value }
        #expect(RingBuilderTests.collected(buffer) == [42])
    }
...
extension RingBuilderTests.Integration {
    ...
    func `Builder result is mutable - pushBack continues sequence`() {
        var buffer: Buffer<Int>.Ring = Buffer<Int>.Ring {
            1
            2
            3
        }
...
extension RingBuilderTests.NonCopyable {
    ...
    func `Builder with single noncopyable element`() {
        let buffer: Buffer<Move>.Ring = Buffer<Move>.Ring {
            Move(42)
        }
        #expect(RingBuilderTests.collected(buffer) == [42])
    }
...
extension RingBuilderTests.StaticMethods {
    ...
    func `buildExpression single element`() {
        let result = Buffer<Int>.Ring.Builder.buildExpression(42)
        #expect(RingBuilderTests.collected(result) == [42])
    }

    @Test
    func `buildExpression existing buffer`() {
        let input: Buffer<Int>.Ring = Buffer<Int>.Ring { 1; 2; 3 }
        let result = Buffer<Int>.Ring.Builder.buildExpression(input)
    ...
    func `buildExpression optional - some`() {
        let value: Int? = 42
        let result = Buffer<Int>.Ring.Builder.buildExpression(value)
        #expect(RingBuilderTests.collected(result) == [42])
    }
    ...
    func `buildPartialBlock first`() {
        let first: Buffer<Int>.Ring = Buffer<Int>.Ring { 1; 2; 3 }
        let result = Buffer<Int>.Ring.Builder.buildPartialBlock(first: first)
        #expect(RingBuilderTests.collected(result) == [1, 2, 3])
    }

    @Test
    func `buildPartialBlock first void`() {
        let result = Buffer<Int>.Ring.Builder.buildPartialBlock(first: ())
        let isEmpty = result.isEmpty
        #expect(isEmpty)
    }
    ...
    func `buildPartialBlock accumulated and next preserves order`() {
        let acc: Buffer<Int>.Ring = Buffer<Int>.Ring { 1; 2 }
        let next: Buffer<Int>.Ring = Buffer<Int>.Ring { 3; 4 }
        let result = Buffer<Int>.Ring.Builder.buildPartialBlock(
            accumulated: acc,
            next: next
        )
    ...
    func `buildBlock empty`() {
        let result = Buffer<Int>.Ring.Builder.buildBlock()
        let isEmpty = result.isEmpty
        #expect(isEmpty)
    }
    ...
    func `buildOptional some`() {
        let component: Buffer<Int>.Ring? = Buffer<Int>.Ring { 1; 2 }
        let result = Buffer<Int>.Ring.Builder.buildOptional(component)
        #expect(RingBuilderTests.collected(result) == [1, 2])
    }

    @Test
    func `buildOptional none`() {
        let component: Buffer<Int>.Ring? = nil
        let result = Buffer<Int>.Ring.Builder.buildOptional(component)
        let isEmpty = result.isEmpty
        #expect(isEmpty)
    }
    ...
    func `buildEither first`() {
        let first: Buffer<Int>.Ring = Buffer<Int>.Ring { 1; 2 }
        let result = Buffer<Int>.Ring.Builder.buildEither(first: first)
        #expect(RingBuilderTests.collected(result) == [1, 2])
    }

    @Test
    func `buildEither second`() {
        let second: Buffer<Int>.Ring = Buffer<Int>.Ring { 3; 4 }
        let result = Buffer<Int>.Ring.Builder.buildEither(second: second)
        #expect(RingBuilderTests.collected(result) == [3, 4])
    }

    @Test
    func `buildLimitedAvailability passthrough`() {
        let component: Buffer<Int>.Ring = Buffer<Int>.Ring { 1; 2; 3 }
        let result = Buffer<Int>.Ring.Builder.buildLimitedAvailability(component)
        #expect(RingBuilderTests.collected(result) == [1, 2, 3])
    }
}
