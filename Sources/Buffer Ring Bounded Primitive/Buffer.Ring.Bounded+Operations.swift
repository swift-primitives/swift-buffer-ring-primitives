...
extension Buffer.Ring.Bounded where S: ~Copyable {
    ...
    @usableFromInline
    mutating func _pushBack<E: ~Copyable>(_ element: consuming E) -> E? where S == Storage<Memory.Allocator<Memory.Heap>>.Contiguous<E> {
    ...
    @usableFromInline
    mutating func _popFront<E: ~Copyable>() -> E where S == Storage<Memory.Allocator<Memory.Heap>>.Contiguous<E> {
        Buffer.Ring.popFront(header: &header, storage: &storage)
    }

    @usableFromInline
    mutating func _pushFront<E: ~Copyable>(_ element: consuming E) -> E? where S == Storage<Memory.Allocator<Memory.Heap>>.Contiguous<E> {
        if header.isFull { return element }
    ...
    @usableFromInline
    mutating func _popBack<E: ~Copyable>() -> E where S == Storage<Memory.Allocator<Memory.Heap>>.Contiguous<E> {
