...
/// `__ColumnDirect` marker, [DS-028] law 1). Its capacity twin ``BoundedTwin`` is the
/// fixed-capacity `Buffer.Ring.Bounded`, through which the column-PRESERVING `.Bounded` alias
/// maps ([DS-028] law 2).
///
...
extension Buffer.Ring: __ColumnDirect where S: Store.Ledgered.`Protocol`, S: ~Copyable {
    public typealias BoundedTwin = Buffer.Ring.Bounded
}
